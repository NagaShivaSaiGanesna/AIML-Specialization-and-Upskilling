...
def ask_ollama(prompt, system_msg, history):
    ...
    #The stream=True means "don't wait for the full answer, send it to me piece by piece."
    response = requests.post(OLLAMA_URL, json=payload, stream=True)

    full_reply = ""
    for line in response.iter_lines():
        #What happens: Sometimes Ollama sends empty lines (like heartbeats), so skip them.
        if not line:
            continue
